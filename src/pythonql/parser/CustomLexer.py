...
from pythonql.parser.Errors import IndentationErr
from pythonql.parser.PythonQLLexer import PythonQLLexer
...
class CustomLexer(PythonQLLexer):
    ...
    def nextToken(self):
        ...
        if self._input.LA(1) == Token.EOF and len(self.indents) != 0:
            ...
            i = len(self.tokens) - 1
            while i >= 0:
                if self.tokens[i].type == PythonQLParser.EOF:
                    del self.tokens[i]
                i -= 1

            # First emit an extra line break that serves as the end of the statement
            self.emitToken(self.commonToken(PythonQLParser.NEWLINE, '\n'))
            ...
            self.emitToken(self.commonToken(PythonQLParser.EOF, "<EOF>"))
    ...
    def createDedent(self):
        dedent = self.commonToken(PythonQLParser.DEDENT, "")
    ...
    def newLineAction(self):
        ...
        if indent == previous:
            ...
            self.emitToken(self.commonToken(PythonQLParser.INDENT, spaces))
        else:
